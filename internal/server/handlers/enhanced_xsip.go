...
// REMOVE these lines:
// var enhancedXsipService *services.SOAPClientService
// func init() { ... }
...
func EnhancedXSIPCancellationHandler(w http.ResponseWriter, r *http.Request) {
	...
	enhancedXsipService, err := services.NewSOAPClientService(logger)
	if err != nil {
...
func validateEnhancedXSIPRequest(req *services.EnhancedXSIPCancellationRequest) error {
	...
	if req.RegnNo == 0 {
		return fmt.Errorf("XSIP registration number should not be blank")
	}
